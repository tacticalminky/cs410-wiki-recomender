...
def create_vocab() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    vocab.to_csv(VOCAB_FILE, header=False, compression='gzip')
...
def reduce_and_sort(inv_idx: pd.DataFrame, vocab: pd.DataFrame) -> None:
    ...
    inv_idx.to_csv(INV_IDX_FILE, header=False, compression='gzip')
    ...
    doc_info.to_csv(DOC_INFO_FILE, header=False, compression='gzip')
